...
function MatrixBoard({ weight, height, orientation, matrix, onCellClick }: { weight: any; height: any; orientation: any; matrix: any; onCellClick: any }) {
    ...
    for (let h: number = 0; h < height; h++) {
        ...
        for (let w: number = 0; w < weight; w++) {
            ...
            if (h === 0 && w === 0) {
                rows.push(
                    <button onClick={onCellClick?.bind(null, h, w)} key={`${w}-${h}`} className={`cell cellRobot ${cellClass}`}>
                        {cellValue}
                ...
                rows.push(
                    <button onClick={onCellClick?.bind(null, h, w)} key={`${w}-${h}`} className={`cell ${cellClass}`}>
                        {cellValue}
                    </button>
                );
            }
